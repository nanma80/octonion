...
def generate_loop(generators, generation_limit = 10):
  ...
  j = generators[1]
  h = generators[2]
  seed = [2] + [0] * 7 # 7062209668
  # seed = times(j, times(i, h))  # 21836768
  # seed = times(times(j, times(i, h)), times(i, h)) # -2384805304
  # seed = [0] * 4 + [2] + [0] * 3 # 1752208380
  # seed = times(times(h, j), times(h, i)) # 1841079592

  print(seed)
  elements = set([tuplize(seed)])
...
print(sum(hash(element) for element in elements))
